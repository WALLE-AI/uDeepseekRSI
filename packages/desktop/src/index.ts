...
import { DirectBackendManager } from '@process/backend';
...
const backendManager = new DirectBackendManager();
...
const handleAppReady = async (): Promise<void> => {
   ...
   *
   * 两个值通过进程继承交给 direct dsh 子进程。继承是启动 ACP 进程时的快照，因此必须
   * 先准备 CDP 凭证，再启动 dsh-bridge；否则内置浏览器 MCP 无法取得凭证。
   *
   ...
   *
   * MUST run before startBackendOrExit(). Both values reach the direct dsh child by process
   * inheritance, which is a snapshot taken when dsh-bridge starts the ACP process. Moving
   * this block after backend startup would leave the browser MCP without credentials.
   *
  ...
  if (cdpStartupEnabled) {
    try {
      ...
      const bridge = await startCdpBridge({
        onTargetRequired: () => {
          console.log('[CDP] Agent requested a browser target; opening the in-app browser panel.');
  ...
  if (debugBackendStartupFailure) {
    ...
    // Start the direct dsh bridge only after Electron-local storage is ready.
      ...
      const handle = await startWebHost({
        ...
        backend: {
          ...
          port: (() => {
            ...
            if (!port) {
              throw new Error('[WebUI] Cannot start: the direct dsh backend is not running.');
            }
            return port;
          })(),
        },
...
// instance lock. A lock-losing instance must not spawn a competing direct backend
// because both instances would own the same dsh sessions and persistence file.
...
installQuitCleanup({
  ...
  // Stop the direct backend; its shutdown disposes the dsh ACP child.

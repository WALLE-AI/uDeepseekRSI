...
/**
 * Migration v26 -> v27: Add the durable dsh turn/update journal.
 * A terminal renderer event is emitted only after these rows and the final
 * assistant message commit together.
 */
const migration_v27: IMigration = {
  version: 27,
  name: 'Add dsh bridge turn journal',
  up: (db) => {
    db.exec(`CREATE TABLE IF NOT EXISTS dsh_turns (
      id TEXT PRIMARY KEY,
      conversation_id TEXT NOT NULL,
      acp_session_id TEXT,
      state TEXT NOT NULL CHECK(state IN ('queued', 'running', 'settling', 'completed', 'interrupted', 'failed')),
    ...
    db.exec('CREATE INDEX IF NOT EXISTS idx_dsh_turns_conversation ON dsh_turns(conversation_id, started_at)');
    db.exec('CREATE INDEX IF NOT EXISTS idx_dsh_turns_state ON dsh_turns(state)');
    db.exec(`CREATE TABLE IF NOT EXISTS dsh_updates (
      ...
      created_at INTEGER NOT NULL,
      FOREIGN KEY (turn_id) REFERENCES dsh_turns(id) ON DELETE CASCADE,
      UNIQUE(turn_id, sequence)
    )`);
    db.exec('CREATE INDEX IF NOT EXISTS idx_dsh_updates_turn ON dsh_updates(turn_id, sequence)');
  },
  down: (db) => {
    db.exec('DROP INDEX IF EXISTS idx_dsh_updates_turn');
    db.exec('DROP TABLE IF EXISTS dsh_updates');
    db.exec('DROP INDEX IF EXISTS idx_dsh_turns_state');
    db.exec('DROP INDEX IF EXISTS idx_dsh_turns_conversation');
    db.exec('DROP TABLE IF EXISTS dsh_turns');
  },
};

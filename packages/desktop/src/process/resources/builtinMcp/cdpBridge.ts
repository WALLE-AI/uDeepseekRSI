...
export type CdpBridgeOptions = {
  /** Ask the renderer to create/show the in-app browser when an agent connects before a target exists. */
  onTargetRequired?: () => void | Promise<void>;
  /** How long an agent command waits for the renderer's webview to become ready. */
...
let targetAttachedWaiters = new Set<() => void>();
...
const attachInternal = (webContentsId: number): { ok: true } | { ok: false; reason: string } => {
  ...
  for (const resolve of targetAttachedWaiters) resolve();
  targetAttachedWaiters.clear();
  return { ok: true };
};

const waitForTargetAttachment = (timeoutMs: number): Promise<boolean> => {
  ...
  return new Promise((resolve) => {
    ...
    const finish = () => {
      ...
      settled = true;
      clearTimeout(timer);
      targetAttachedWaiters.delete(finish);
      resolve(Boolean(attached && !attached.contents.isDestroyed()));
    };
    const timer = setTimeout(finish, timeoutMs);
    targetAttachedWaiters.add(finish);

    // Cover the narrow race where attachInternal completed between the first check
    // and registering this waiter.
    if (attached && !attached.contents.isDestroyed()) finish();
  });
};
...
const handleSocketMessage = async (
  ...
  ensureTargetAttached: () => Promise<boolean>
) => {
  let req: CdpRequest;
  try {
  ...
  // forward
  if ((!attached || attached.contents.isDestroyed()) && !(await ensureTargetAttached())) {
    /**
    ...
    sendError(
      ws,
      id,
      'The in-app browser could not be opened or attached. Open the browser panel in uworker and try again.',
      sessionId
    );
    return;
  }
...
export const startCdpBridge = async (options: CdpBridgeOptions = {}): Promise<CdpBridgeHandle> => {
  ...
  let targetRequest: Promise<boolean> | null = null;

  const ensureTargetAttached = (): Promise<boolean> => {
    if (attached && !attached.contents.isDestroyed()) return Promise.resolve(true);
    if (!options.onTargetRequired) return Promise.resolve(false);
    if (targetRequest) return targetRequest;

    targetRequest = (async () => {
      const waiting = waitForTargetAttachment(targetAttachTimeoutMs);
      try {
        await options.onTargetRequired?.();
      } catch (error) {
        console.warn('[CDP] Failed to request the in-app browser panel:', error);
      }
      return waiting;
    })().finally(() => {
      targetRequest = null;
  ...
  httpServer.on('upgrade', (req, socket, head) => {
    ...
    wss.handleUpgrade(req, socket, head, (ws) => {
      ...
      ws.on(
        'message',
        (data) => void handleSocketMessage(ws, data.toString(), announcedSessions, ensureTargetAttached)
      );

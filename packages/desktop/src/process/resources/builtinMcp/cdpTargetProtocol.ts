...
export const buildVersionPayload = (wsUrl: string, chromeVersion: string) => ({
  ...
  'User-Agent': `uworker in-app browser (Chrome/${chromeVersion})`,
...
export const decideCdpCommand = (req: CdpRequest, getTargetInfo: () => TargetInfo): CdpDecision => {
  ...
  switch (method) {
    ...
    case 'Target.createTarget':
      return {
        kind: 'error',
        message: 'uworker in-app browser exposes a single fixed tab; Target.createTarget is not supported.',
      };

    case 'Target.createBrowserContext':
    case 'Target.disposeBrowserContext':
      return { kind: 'error', message: 'uworker in-app browser does not support multiple browser contexts.' };
     ...
     */
    case 'Browser.close':
      return { kind: 'error', message: 'Browser.close is not permitted against the uworker in-app browser.' };

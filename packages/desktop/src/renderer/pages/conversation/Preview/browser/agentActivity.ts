 ...
 */
const IN_FLIGHT_STATUSES = new Set(['Executing', 'Pending', 'Confirming', 'pending', 'in_progress']);
...
const isBrowserMcpEntry = (entry: ToolGroupEntry): boolean => {
  ...
  if (
    typeof name === 'string' &&
    (name.startsWith(`${BUILTIN_BROWSER_MCP_NAME}__`) || name.startsWith(`mcp__${BUILTIN_BROWSER_MCP_NAME}__`))
  ) {
    return true;
  }
...
const toolEntries = (messageType: string, data: unknown): ToolGroupEntry[] => {
  if (messageType === 'tool_group') return Array.isArray(data) ? (data as ToolGroupEntry[]) : [];
  if (messageType !== 'acp_tool_call' || !data || typeof data !== 'object') return [];
  const envelope = data as Record<string, unknown>;
  const update =
    envelope.update && typeof envelope.update === 'object' ? (envelope.update as Record<string, unknown>) : envelope;
  const sessionUpdate = String(update.sessionUpdate ?? update.session_update ?? '');
  return [
    {
      name: String(update.name ?? update.title ?? ''),
      status: String(update.status ?? (sessionUpdate === 'tool_call' ? 'in_progress' : 'completed')),
      confirmationDetails:
        update.confirmationDetails && typeof update.confirmationDetails === 'object'
          ? (update.confirmationDetails as Record<string, unknown>)
          : undefined,
    },
  ];
};
...
export const isBrowserMcpActivity = (messageType: string, data: unknown): boolean => {
  return toolEntries(messageType, data).some(
...
export const isBrowserMcpSettled = (messageType: string, data: unknown): boolean => {
  const entries = toolEntries(messageType, data).filter(isBrowserMcpEntry);

...
import { Copy, Robot } from '@icon-park/react';
...
const MessageText: React.FC<{
  ...
  const assistantLabel = conversationContext?.assistantName || conversationContext?.assistantBackend || 'uworker';
  const assistantBackendLogo = conversationContext?.assistantBackend
    ? resolveAgentLogo(logos, { backend: conversationContext.assistantBackend })
    : null;
  const assistantIdentity = showAssistantIdentity ? (
    <div className='mb-6px flex h-24px items-center gap-7px' data-testid='assistant-message-identity'>
      <span className='h-24px w-24px flex flex-shrink-0 items-center justify-center overflow-hidden rounded-6px bg-fill-2'>
        {conversationContext?.assistantAvatarIsFallback ? (
          <Robot theme='outline' size={14} />
        ) : conversationContext?.assistantAvatarIsEmoji ? (
          <span className='text-15px leading-none' role='img' aria-label={assistantLabel}>
            {conversationContext.assistantAvatar}
          </span>
        ) : conversationContext?.assistantAvatar || assistantBackendLogo ? (
          <ThemedLogo
            src={conversationContext?.assistantAvatar || assistantBackendLogo}
            alt={assistantLabel}
      ...
      <span className='min-w-0 truncate text-12px font-medium text-t-secondary'>{assistantLabel}</span>
    </div>
  ) : null;

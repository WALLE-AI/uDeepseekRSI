...
const WebviewHost: React.FC<WebviewHostProps> = ({
  ...
  useEffect(() => {
    ...
    // depend on winning that one-shot event race.
    const timers = [0, 50, 150, 500, 1_500].map((delay) => window.setTimeout(() => reportBrowserWebContents(), delay));
    return () => timers.forEach((timer) => window.clearTimeout(timer));
  }, [agentBrowserControl, agentBrowserControlActive, reportBrowserWebContents]);

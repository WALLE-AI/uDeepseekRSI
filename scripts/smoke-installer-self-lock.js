...
function main() {
  ...
  const nsi = `
Unicode true
Name "uworker Installer Self Lock Smoke"
...
!define AIONUI_APP_EXECUTABLE_FILENAME "uworker.exe"
!define UNINSTALL_FILENAME "Uninstall uworker.exe"
...
`;
  ...
  try {
    ...
    if (!blocking.some((process) => process.name === 'uworker installer' && Number(process.pid) > 0)) {
      throw new Error(`expected uworker installer blocker, got ${JSON.stringify(blocking)}`);
    }

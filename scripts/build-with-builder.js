...
/**
 * Simplified build script for uworker
...
try {
  ...
  // 5. Prepare resources for the direct DeepSeek Harness runtime.
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const uworkerRunning = isProcessRunningWindows('uworker.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (uworkerRunning || electronRunning) {
        console.log('⚠️  Detected running uworker/Electron process. Attempting to close...');
        killWindowsProcesses(['uworker.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running uworker/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'uworker.exe');
    ...
    console.log('⚠️  Windows local build failed after uworker.exe was produced.');
    if (firstError) {

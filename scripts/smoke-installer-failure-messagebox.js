...
const INSTALLER_ERROR_SCENARIOS = [
  {
    ...
    code: 'E1001',
    message: 'uworker could not repair the installed uninstaller.',
    action: 'Close uworker, restart Windows if needed, then run this installer again.',
    diagnostics:
  ...
  },
  {
    ...
    code: 'E1002',
    message: 'The previous uworker uninstaller returned an error.',
    action:
  ...
  },
  {
    ...
    code: 'E1003',
    message: 'uworker could not remove or replace the previous installation directory.',
    action: 'Close uworker and any program using the install folder, then run this installer again.',
  ...
  },
  {
    ...
    code: 'E1010',
    message: 'uworker could not extract the application files correctly.',
    action: 'Download a fresh installer and run it again.',
    diagnostics: 'scenario=extract-failed phase=extract method=zip missing=uworker.exe',
  },
  {
    id: 'disk-insufficient',
    defineName: 'AIONUI_E_DISK_INSUFFICIENT',
    code: 'E1020',
    message: 'uworker cannot continue because the target disk does not have enough free space.',
  ...
  },
  {
    ...
    code: 'E1030',
    message: 'uworker installed, but the bundled AionCore resources are incomplete.',
    action: 'Download a fresh installer and run it again.',
  ...
  },
  {
    ...
    code: 'E1031',
    message: 'uworker installation is incomplete because a required application file is missing.',
    action: 'Reinstall uworker or download a newer installer.',
  ...
  },
  {
    ...
    action: 'Download the uworker installer that matches this Windows architecture, then run it again.',
  ...
  },
  {
    ...
    code: 'E1050',
    message: 'Another uworker installer appears to still be active.',
  ...
  },
  {
    ...
    code: 'E1060',
    message: 'uworker found an invalid previous-install registry state.',
    action: 'Uninstall the old uworker from Windows Settings, then run this installer again.',
  ...
  },
  {
    ...
    code: 'E1070',
    message: 'uworker could not write the active-installer marker.',
...
function Find-FailureWindow([string]$Code, [int]$TimeoutSec = 90) {
  ...
  do {
    ...
    foreach ($window in $windows) {
      ...
      if ($text -like "*uworker installation failed ($Code)*" -or
...
try {
  ...
  foreach ($required in @(
    "uworker installation failed ($Code)",
    "scenario=$ScenarioId",
    'Suggested action:',
    'Diagnostics:',
    'Installer log:',
...
function createHarnessNsi({ exePath, logPath, projectRoot, scenario }) {
  ...
  return `
Unicode true
Name "uworker Failure MessageBox Smoke"
...
Section
  StrCpy $INSTDIR "$TEMP\\uworker-messagebox-smoke"
  ...
  try {
    ...
    if (autoDecline) {
      ...
      if (autoDecline === 'consent') {
        ...
        if (typeof status.copyText !== 'string' || !status.copyText.includes(`uworker installer failure ${code}`)) {

 ...
 *      (e.g. out/linux-unpacked/uworker, out/mac-arm64/uworker.app, out/win-unpacked/uworker.exe).
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/uworker.exe  or  out/win-x64-unpacked/uworker.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'uworker.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/uworker.app/Contents/MacOS/uworker  or  out/mac/uworker.app/...

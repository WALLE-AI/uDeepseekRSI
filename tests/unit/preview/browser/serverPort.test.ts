...
describe('buildMcpSpawnCommand — issue #3883', () => {
  ...
  it('routes through cmd.exe /c on Windows with npx as an argument', () => {
    const { command, args, windowsHide } = buildMcpSpawnCommand({ platform: 'win32', version, browserUrl });
    expect(command).toBe('cmd.exe');
  ...
  it('keeps the plain npx invocation on POSIX platforms', () => {
    for (const platform of ['darwin', 'linux']) {
      const { command, args, windowsHide } = buildMcpSpawnCommand({ platform, version, browserUrl });
      expect(command).toBe('npx');
      expect(args[0]).toBe('-y');
      expect(args).not.toContain('/c');
      expect(windowsHide).toBe(false);
    }
  });

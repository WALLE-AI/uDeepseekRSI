...
describe('isBrowserMcpActivity', () => {
  ...
  it('falls back to the tool-name prefix when confirmationDetails is absent', () => {
    ...
    expect(
      isBrowserMcpActivity('tool_group', [
        { name: `mcp__${BUILTIN_BROWSER_MCP_NAME}__navigate_page`, status: 'Executing' },
      ])
    ).toBe(true);
  });

  it('detects DeepSeek Harness ACP tool-call updates', () => {
    expect(
      isBrowserMcpActivity('acp_tool_call', {
        update: {
          sessionUpdate: 'tool_call',
          title: `mcp__${BUILTIN_BROWSER_MCP_NAME}__navigate_page`,
        },
      })
    ).toBe(true);
    expect(
      isBrowserMcpSettled('acp_tool_call', {
        update: {

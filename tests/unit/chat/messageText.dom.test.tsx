...
vi.mock('@/renderer/components/agent/ThemedLogo', () => ({
  __esModule: true,
  default: ({ src, alt }: { src?: string; alt: string }) => (
    <img data-testid='assistant-avatar-image' src={src} alt={alt} />
  ),
}));
...
describe('MessageText attachment paths', () => {
  ...
  it('shows the selected assistant avatar and name at the start of a reply', () => {
    const message: IMessageText = {
      id: 'assistant-reply',
      msg_id: 'assistant-reply',
    ...
    render(
      <ConversationProvider
        value={{
          conversation_id: 'conv-1',
          type: 'acp',
          assistantName: 'Research agent',
          assistantAvatar: '/api/assistants/research/avatar',
        }}
      >
        <MessageText message={message} showAssistantIdentity />
      </ConversationProvider>
    );

    expect(screen.getByTestId('assistant-message-identity')).toHaveTextContent('Research agent');
    expect(screen.getByTestId('assistant-avatar-image')).toHaveAttribute('src', '/api/assistants/research/avatar');
  });

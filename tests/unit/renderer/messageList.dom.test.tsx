...
vi.mock('@/renderer/pages/conversation/Messages/components/MessageText', () => ({
  default: ({
    ...
    <div
      ...
      data-assistant-identity={String(showAssistantIdentity ?? false)}
    >
...
describe('MessageList', () => {
  ...
  it('shows the assistant identity once at the first text reply of each turn', () => {
    const first = { ...createTextMessage(), id: 'assistant-1' };
    const continuation = { ...createTextMessage(), id: 'assistant-2', msg_id: 'assistant-2' };
    const user = { ...createTextMessage(), id: 'user-1', msg_id: 'user-1', position: 'right' as const };
    const nextReply = { ...createTextMessage(), id: 'assistant-3', msg_id: 'assistant-3' };

    render(
      <Wrapper messages={[first, continuation, user, nextReply]}>
        <MessageList />
      </Wrapper>
    );

    expect(screen.getByTestId('msgtext-assistant-1')).toHaveAttribute('data-assistant-identity', 'true');
    expect(screen.getByTestId('msgtext-assistant-2')).toHaveAttribute('data-assistant-identity', 'false');
    expect(screen.getByTestId('msgtext-user-1')).toHaveAttribute('data-assistant-identity', 'false');
    expect(screen.getByTestId('msgtext-assistant-3')).toHaveAttribute('data-assistant-identity', 'true');
  });

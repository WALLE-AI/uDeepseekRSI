...
vi.mock('@process/services/database/backupDatabase', () => ({
  backupDatabaseBeforeV27: vi.fn(),
}));
...
describe('configMigrationIntegration', () => {
  ...
  it('runs migrations when database version is outdated', async () => {
    ...
    expect(runMigrations).toHaveBeenCalledWith(mockDriver, 20, 27);
    expect(setDatabaseVersion).toHaveBeenCalledWith(mockDriver, 27);
  });
  ...
  it('runs handoff repair even when database version is current', async () => {
    (getDatabaseVersion as any).mockReturnValue(27);
